...
- **제품 상세** — 건강 등급, 카테고리 내 순위·백분위, 등급 근거(감점·가점 성분 구분), 영양성분표(미측정 `—`와 실제 0 구분)
- **카테고리 순위** — 건강 점수 순 목록. 등급 필터와 "덜 건강한 순" 정렬 지원
- **카테고리 비교** — 9개 카테고리를 평균 점수·점수 범위·D·E 비율·평균 성분으로 나란히 대조
- **집계 분석 대시보드** — 카테고리 단위 등급 분포, 평균, 4성분(당류·나트륨·포화지방·에너지) 상관 표와 "주범 성분" 인사이트, 당류 산점도, 스냅샷 이력
...
scripts/regrade/  재수집 없이 등급·순위·집계만 재계산 (pnpm regrade)
test/             Vitest 단위 + Playwright e2e
docs/adr/         아키텍처 결정 기록(ADR)
docs/analysis-questions.md  가설(H1~H5)·검증 수치·화면 매핑 정의
CONTEXT.md        도메인 용어집
```
...
등급 로직만 바뀌었고 수집 데이터가 그대로라면 재적재 대신 **regrade**를 쓰세요 — DB의
영양성분으로 등급·순위·집계만 다시 계산합니다(수 분 소요, API 호출 없음):
...
> 원본이 수십 MB라 **git에 커밋하지 않고** Releases 자산으로 관리합니다(현재 자산 약 64MB, 비압축).
...
|---|---|---|---|---|
| `data-2026-07-24` | 2026-07-23 | 70,013 (gradable 69,989) | `nutriscore-2023-v1` | 9카테고리 전량 · ADR-0007 교정 반영 |
| `data-2026-07-10` | 2026-07-10 | 48,215 (gradable 48,199) | `nutriscore-2023-v1` | 최초 전량 미러 (6카테고리, ADR-0007 이전) |
...
- [x] 확장·교정 반영 전량 재적재 (`data-2026-07-24` 스냅샷, 70,013건)

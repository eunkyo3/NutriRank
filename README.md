...
데이터 모델·건강 등급·파이프라인·화면 구현이 완료·검증됐습니다. **실데이터 라이브 적재**(15100066 오픈API 엔드포인트 설정 후 `pnpm ingest`)와 식약처 분류 매핑 시드 큐레이션만 남았습니다.
...
- [~] 데이터 파이프라인 (공개 데이터 수집·적재) — 12단계 코어 완성·검증. 라이브 수집·매핑 시드는 15100066 활용신청 승인 후
- [x] 화면 (검색 · 상세 · 순위 · 대시보드) — 사전계산 테이블 조회, 실데이터는 파이프라인 적재 후

> 핵심 로직(`lib/grading`·`scripts/ingest`)과 화면(`app/*`)은 구현·검증 완료입니다.
> 실데이터는 데이터 파이프라인 라이브 적재(15100066 엔드포인트 설정 후 `pnpm ingest`) 시점부터 화면에 흐릅니다.
